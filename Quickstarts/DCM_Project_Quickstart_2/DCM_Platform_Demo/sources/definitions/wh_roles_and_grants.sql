{% for team in teams %}
    ...
    {% if team.raw_access == 'READ' %}
        grant USAGE on database DCM_DEMO_2 to role DCM_DEMO_2_{{team_name}}_ADMIN;
        grant USAGE on schema DCM_DEMO_2.RAW to role DCM_DEMO_2_{{team_name}}_ADMIN;
        grant select on ALL TABLES in schema DCM_DEMO_2.RAW to role DCM_DEMO_2_{{team_name}}_ADMIN;    
        
    {% elif team.raw_access == 'WRITE' %}
        grant USAGE on database DCM_DEMO_2 to role DCM_DEMO_2_{{team_name}}_ADMIN;
        grant USAGE on schema DCM_DEMO_2.RAW to role DCM_DEMO_2_{{team_name}}_ADMIN;
        grant select on ALL TABLES in schema DCM_DEMO_2.RAW to role DCM_DEMO_2_{{team_name}}_ADMIN;
        grant insert, update, delete on ALL TABLES in schema DCM_DEMO_2.RAW to role DCM_DEMO_2_{{team_name}}_ADMIN;
    {% endif %}
    ...
    grant database role SNOWFLAKE.DATA_METRIC_USER to role DCM_DEMO_2_{{team_name}}_ADMIN;
    grant execute data metric function on account to role DCM_DEMO_2_{{team_name}}_ADMIN;
{% endfor %}
